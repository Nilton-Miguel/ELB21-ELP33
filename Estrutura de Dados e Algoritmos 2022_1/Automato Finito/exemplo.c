...
int main(void){
    ...
    /*  Uma máquina que só aceita cadeias de texto que terminem com o 
        caractere '1' num alfabeto de "0 1" */
    ...
    // só retorna 1 pros textos terminados em '1'
    int r1 = processa(mach1, "0\0", 0, inicial);
    int r2 = processa(mach1, "1\0", 0, inicial);

    int r3 = processa(mach1, "00\0", 0, inicial);
    int r4 = processa(mach1, "01\0", 0, inicial);
    int r5 = processa(mach1, "10\0", 0, inicial);
    int r6 = processa(mach1, "11\0", 0, inicial);

    // retorna 0 pra todas as cadeias que contenham um caractere fora do alfabeto
    int r7 = processa(mach1, "55\0", 0, inicial);
    int r8 = processa(mach1, "51\0", 0, inicial);
    int r9 = processa(mach1, "15\0", 0, inicial);

    //-----------------------------------------------------------------------------------

    /*  Uma máquina que só aceita cadeias de texto que terminem com o 
        caractere 'b' num alfabeto de "a b" */

    machine * mach2 = novo_automato();

    // alfabeto de mach1
    insere_caractere(mach2, 'a');
    insere_caractere(mach2, 'b');

    // coleção de estados de mach1
    insere_estado(mach2, 0, "2a", 0);
    insere_estado(mach2, 0, "2b", 1);
    insere_estado(mach2, 1, "2c", 2);
    ...
    inicial = mach2 -> estados -> inicio;

    // só retorna 1 pros textos terminados em '1'
    int p1 = processa(mach2, "a\0", 0, inicial);
    int p2 = processa(mach2, "b\0", 0, inicial);

    int p3 = processa(mach2, "aa\0", 0, inicial);
    int p4 = processa(mach2, "ab\0", 0, inicial);
    int p5 = processa(mach2, "ba\0", 0, inicial);
    int p6 = processa(mach2, "bb\0", 0, inicial);

    // retorna 0 pra todas as cadeias que contenham um caractere fora do alfabeto
    int p7 = processa(mach2, "ee\0", 0, inicial);
    int p8 = processa(mach2, "eb\0", 0, inicial);
    int p9 = processa(mach2, "be\0", 0, inicial);
